...
| 04-01-09b | 01 | 1 | ICN-03 (ROADMAP SC5) | grep + inverse-grep | Positive: `grep -rE "fill\s*=\s*(SolidColor\(Color\.Transparent\)\|Color\.Transparent\|null)" library/src/main/kotlin/com/mordred/aero/icons/internal/ \| wc -l` (must return ≥7 — one transparent-fill statement per spike file; accepts `SolidColor(Color.Transparent)`, `Color.Transparent`, or `null`). Inverse: `grep -rE "fill\s*=\s*SolidColor\(Color\.(Black\|White\|Red\|Green\|Blue)\)" library/src/main/kotlin/com/mordred/aero/icons/internal/ \| wc -l` (must return 0 — no opaque solid fills). | ❌ W0 | ⬜ pending |
...
| 04-02-09 | 02 | 1 | ICN-03 (ROADMAP SC5) | grep + inverse-grep | Positive: `grep -rE "fill\s*=\s*(SolidColor\(Color\.Transparent\)\|Color\.Transparent\|null)" library/src/main/kotlin/com/mordred/aero/icons/internal/ \| wc -l` (must return ≥138 — one transparent-fill statement per file at minimum across the full generated tree). Inverse: `grep -rE "fill\s*=\s*SolidColor\(Color\.(Black\|White\|Red\|Green\|Blue)\)" library/src/main/kotlin/com/mordred/aero/icons/internal/ \| wc -l` (must return 0 — no opaque solid-color fills anywhere). Supplementary manual spot-read of 5 random files (Archive, Bell, Envelope, House, Warning) confirms the per-file presence of `viewportWidth=256f`, `strokeLineWidth=16f`, `StrokeCap.Round`, `StrokeJoin.Round` alongside the transparent fill — but the ROADMAP SC5 gate is the all-files automated grep above. | ❌ W0 | ⬜ pending |
